...
| **통화정책 일정** | 정적 데이터 파일(`docs/cbank.js`) | 수동 — Claude에게 "통화정책 일정 업데이트해줘" 요청 | 없음 |

즉 SR지수만 "서버가 미리 만들어둔 값"이고, 나머지 거시지표는 "당신이 페이지를 여는 순간 당신의 브라우저가 직접" 가져옵니다.
...
docs/                    ← GitHub Pages가 서빙하는 폴더 (Settings → Pages → main /docs)
  ...
  cbank.js                ← 중앙은행 통화정책 일정·결과 (정적, 수동 갱신)
...
2. **API 키 3종 발급** (모두 무료, SR지수 서버 빌드용)
...
물가·통화/고용/경기·심리/PCE확산/통화정책 탭은 위 세팅과 무관하게 Pages 활성화 즉시 동작합니다(4~6단계는 SR지수 탭 전용).
...
- **통화정책 일정**: 각 중앙은행 회의 직후, Claude에 "통화정책 일정 업데이트해줘" 요청 → `docs/cbank.js`에 결과·발효일 기입.

## 자동화 방식 검토 (요약)

- **더 자동화할 수 있는 것 없음**: SR지수(서버 빌드)와 물가·통화/고용/경기·심리(브라우저 실시간 조회)는 이미 사람 개입 없이 갱신됩니다.
- **구조적으로 완전 자동화가 불가능한 것**: PCE 확산 지표(BEA 2.4.4U는 API가 아니라 엑셀 배포, 매월 발표일도 비정기), 통화정책 결과(회의 종료 후 사람이 보도자료를 교차검증해 기입하는 규약 — methodology_external.md 4장). 자동 크롤러로 대체 시 오탈자·조기 확정 리스크가 커서 의도적으로 수동 게이트를 둔 설계입니다.
